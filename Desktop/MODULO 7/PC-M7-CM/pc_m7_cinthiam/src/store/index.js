...
export default new Vuex.Store({
  ...
  mutations: {
    AGR_CURSO:(state, nuevoCurso)=>{
      let exist = state.cursos.some(nuevo=>nuevo.id==nuevoCurso.id)
      ...
      if(exist==false){
        ...
        state.cursos.forEach(nuevo=>{
          if(nuevo.id == nuevoCurso.id){
            nuevo.count+=1;
          }
  ...
  actions: {
    agrCurso:({commit},nuevoCurso)=>{
      commit('AGR_CURSO',nuevoCurso)
    },
  },
  modules: {}
})
